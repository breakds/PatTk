...
namespace PatTk
{
  ...
  template <typename T>
  int SSLKernel<T>::numHypo = 10;

  template <typename T>
  int SSLKernel<T>::stopNum = 1;

  template <>
  double SSLKernel<float>::converge = 0.001;

  template <>
  double SSLKernel<double>::converge = 0.001;

  template <>
  int SSLKernel<unsigned char>::converge = 10;
